...
- (void)insertData:(NSDictionary *)json intoPageWithVarName:(NSString *)appProperty
{
    ...
    if (contentParseError == nil && objectOfJSON) {
        ...
        [self executeJavaScriptString:[NSString stringWithFormat:@"if(window.bridgeDebug){window.bridgeDebug.%@ = %@;}", appProperty, str]];
    }
}
...
{
    [self.webView evaluateJavaScript:[NSString stringWithFormat:@"window.bd_eval(%@)", jsCode] completionHandler:^(NSDictionary *data, NSError * _Nullable error) {
        if (completion) {
...
- (void)__execScript:(NSString *)actionName funcName:(NSString *)funcName param:(NSDictionary *)paramDict
{
    ...
    NSString *jsCode = [NSString stringWithFormat:@"window.bridgeDebug.%@('%@',%@);", funcName, actionName, [[NSString alloc] initWithData:objectOfJSON encoding:NSUTF8StringEncoding]];
...
+ (void)prepareJavaScript:(id)script when:(WKUserScriptInjectionTime)injectTime key:(NSString *)key
{
    ...
    if ([script isKindOfClass:NSString.class]) {
        ...
        if(urlToRequest){
            ...
            result = [NSString stringWithFormat:bd_ml((function(e){
                e.setAttribute("src",'%@');
        ...
        BDDebugLog(@"fail to inject javascript");
    }
}
...
- (void)injectScriptsToUserContent:(WKUserContentController *)userContentController
{
    ...
    if (kAppHostSource == nil) {
        NSURL *jsLibURL = [[bundle bundleURL] URLByAppendingPathComponent:@"bridgeDebug_version_1.5.0.js"];
        kAppHostSource = [NSString stringWithContentsOfURL:jsLibURL encoding:NSUTF8StringEncoding error:nil];
        [self.class _addJavaScript:kAppHostSource when:WKUserScriptInjectionTimeAtDocumentStart forKey:@"bridgeDebug.js"];
