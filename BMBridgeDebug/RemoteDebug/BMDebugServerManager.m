...
#import "BMDebugServerManager.h"
#import "GCDWebServer.h"
#import "GCDWebServerDataResponse.h"
#import "BDDebugViewController.h"
...
@property (nonatomic, strong) BDDebugViewController *debugVC;
...
@implementation BMDebugServerManager {
...
{
    static BMDebugServerManager *_manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _manager = [BMDebugServerManager new];
...
- (void)debugCommand:(NSString *)action param:(NSDictionary *)param {
    if (action.length > 0) {
        ...
        BDDebugLog(@"irregular param %@",param);
    }
}
...
#define kOldWindowPostion CGRectMake(BD_SCREEN_WIDTH - 60, 150, kDebugWinInitWidth, kDebugWinInitHeight)
...
{
    if (self.debugVC == nil) {
        BDDebugViewController *vc = [[BDDebugViewController alloc] init];
        self.debugVC = vc;
...
- (void)start
{
    ...
    [_webServer addDefaultHandlerForMethod:@"POST"
                              ...
                              processBlock:^GCDWebServerResponse *(GCDWebServerURLEncodedFormRequest *request) {
                                  ...
                                  if ([url.path hasPrefix:@"/react_log.do"]) {
                                      ...
                                      NSString *action = [request.arguments objectForKey:kBDActionKey];
                                      NSString *param = [request.arguments objectForKey:kBDParamKey] ?: @"";
                                      ...
                                      if (action.length > 0) {
                                          if ([NSThread isMainThread]) {
                                              ...
                                              BDDebugLog(@"switch to main thread");
                                              dispatch_async(dispatch_get_main_queue(), ^{
                                                  [strongSelf debugCommand:action param:contentJSON];
                                              });
                                          }
                                      } else {
                                          BDDebugLog(@"command.do arguments error");
                                      }
                                      //
                                  }
    ...
    [_webServer startWithPort:12345 bonjourName:@"birdmichael-mac.local"];
